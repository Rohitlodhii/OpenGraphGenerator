...
type Props = {
  ...
  onUse: (motif: MotifDef, result: ImportResult) => void
}

const MotifsDialog: React.FC<Props> = ({ open, onOpenChange, onUse }) => {
  ...
  const handleUse = React.useCallback(async () => {
    const motif = motifs.find((m) => m.id === selectedId)
    if (!motif || loading) return
    setLoading(true)
    setError(null)
    try {
      const res = await fetch(motif.src)
      if (!res.ok) throw new Error(`Failed to load ${motif.name}`)
      const text = await res.text()
      const result = importSVG(text)
      if (!result.success || !result.root) {
        throw new Error(result.error ?? "Could not parse motif")
      }
      onUse(motif, result)
      onOpenChange(false)
    } catch (err) {
  ...
  const header = (
    <div className="flex flex-col gap-1 border-b border-border bg-sidebar px-6 py-4">
      <h2 className="font-heading font-semibold text-xl leading-none">Motifs</h2>
      <p className="text-muted-foreground text-sm">
        Pick a motif and drop it on your canvas. Each part stays recolorable and editable.
      </p>
    </div>
  )
