...
export default function ShaderBackground() {
  ...
  useEffect(() => {
    ...
    // Hex to linear RGB float values:
    ...
    // Constant uniform vectors matching specs:
    // u_shape = vec4(1.68, 0.72, 0.50, 0.00) -> scale, intensity, paramA, warp
    // u_surface = vec4(2.40, 1.08, 0.00, 1.00) -> detail, contrast, brightness, saturation
    // u_finish = vec4(0.00, 0.00, 0.000, 0.08) -> hue, vignette, blur, grain
    // u_transform = vec4(8617.0, 0.00, 0.00, 0.0) -> seed, rotation, drift, OKLab toggle
    // u_space = vec4(0.00, 0.00, 0.0, 0.0) -> offset.xy, pointer.xy
    // u_cursor = vec4(0.0, 2.0, 0.65, 0.46) -> cursor presence 0 (off)
    if (uShapeLoc) gl.uniform4f(uShapeLoc, 1.68, 0.72, 0.50, 0.00);
    if (uSurfaceLoc) gl.uniform4f(uSurfaceLoc, 2.40, 1.08, 0.00, 1.00);
    if (uFinishLoc) gl.uniform4f(uFinishLoc, 0.00, 0.00, 0.00, 0.08);
    if (uTransformLoc) gl.uniform4f(uTransformLoc, 8617.0, 0.00, 0.00, 0.0);
    ...
    const render = () => {
      ...
      const timeVal = elapsedSeconds * 0.76;
